...
public class HighScorePanel extends BackgroundPanel {
	...
	private final static float FONTSIZE = 20.0f;
	...
	public HighScorePanel(DungeonMap dm, int numListEntries, ActionListener backToMainMenuListener) {
		super(new ImageIcon(
				HighScorePanel.class.getClassLoader().getResource("images/background/highscore/HighScoreBG.jpg"))
						.getImage());
	...
	private void buildGui(ActionListener backToMainMenuListener) {
		...
		for (int i = 0; i < numListEntries; i++) {
			...
			names[i].setFont(names[i].getFont().deriveFont(FONTSIZE));
			dashes[i] = new JLabel();
			dashes[i].setBounds(new Rectangle(x2, yStart + i * yBorder, 200, 30));
			dashes[i].setForeground(Color.WHITE);
			dashes[i].setFont(names[i].getFont().deriveFont(FONTSIZE));
			scores[i] = new JLabel();
			...
			scores[i].setFont(names[i].getFont().deriveFont(FONTSIZE));
		...
		mmenu.addMouseListener(new MouseAdapter() {
			...
			public void mouseEntered(MouseEvent e) {
				mmenu.setIcon(
						new ImageIcon(MainMenuPanel.class.getResource("/images/background/highscore/back-shine.png")));
			}
